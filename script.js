...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function typeWriter() {
        if (titleElement && charIndex < fullTitle.length) {
    ...
    async function handleSearch() {
        if (!codeInput.value) { alert('Please enter a Member ID.'); return; }
    ...
    function applyTheme(theme) {
        ...
        if (themeToggle) themeToggle.innerHTML = theme === 'light' ? '<i class="fas fa-moon"></i>' : '<i class="fas fa-sun"></i>';
    }

    if (searchBtn) searchBtn.addEventListener('click', handleSearch);
    if (codeInput) codeInput.addEventListener('keypress', (event) => { if (event.key === "Enter") searchBtn.click(); });
